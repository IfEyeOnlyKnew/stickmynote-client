...
function createWebSocketServer(server) {
  ...
  // Handle upgrade requests manually for full control + diagnostics
  server.on("upgrade", (req, socket, head) => {
    console.log(`[WebSocket] Upgrade event fired for ${req.url}`)
    console.log(`[WebSocket] Socket state: readable=${socket.readable} writable=${socket.writable} destroyed=${socket.destroyed}`)
    ...
    verifyToken(token)
      .then((auth) => {
        ...
        console.log(`[WebSocket] Pre-upgrade socket: readable=${socket.readable} writable=${socket.writable} destroyed=${socket.destroyed}`)

        wss.handleUpgrade(req, socket, head, (ws) => {
          console.log(`[WebSocket] Upgrade complete for ${auth.userId}`)
          ws.wsAuth = { userId: auth.userId, orgIds: auth.orgIds }
