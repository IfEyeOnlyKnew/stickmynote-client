...
export function DecisionCockpitSidebar({ isOpen, onClose, onFilterSelect }: Readonly<DecisionCockpitSidebarProps>) {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true)
      const response = await fetch("/api/inference-analytics/workflow")
      if (response.ok) {
  ...
  const handleExport = () => {
    ...
    a.href = url
    a.download = `inference-hub-metrics-${new Date().toISOString().split("T")[0]}.json`
    a.click()
          ...
          {!loading && (data ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 px-4">
              ...
              <p className="text-muted-foreground text-center">
                No workflow data available yet. Start creating and managing inference sticks to see analytics.
              </p>
            </div>
          ))}
        </div>
