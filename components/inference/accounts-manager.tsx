...
export function AccountsManager() {
  ...
  const fetchAccounts = async () => {
    try {
      const response = await fetch("/api/inference-accounts")
      if (response.ok) {
  ...
  const handleImportCSV = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/inference-accounts/bulk", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteAccount = async (accountId: string) => {
    ...
    try {
      const response = await fetch(`/api/inference-accounts/${accountId}`, {
        method: "DELETE",
      })
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Add contacts that can be invited to your inference pads. You can add them individually or import via CSV.
          </CardDescription>
        </CardHeader>
        <CardContent>
