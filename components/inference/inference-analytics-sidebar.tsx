...
interface InferenceAnalyticsSidebarProps {
  isOpen: boolean
  onClose: () => void
}
...
export function InferenceAnalyticsSidebar({ isOpen, onClose }: Readonly<InferenceAnalyticsSidebarProps>) {
  ...
  const fetchAnalytics = async () => {
    try {
      setLoading(true)
      const response = await fetch("/api/inference-analytics")
      if (response.ok) {
  ...
  return (
    <div className="fixed inset-0 z-[9998] bg-black/50 transition-opacity duration-300">
      <div
        id="inference-analytics-sidebar"
      ...
      >
        <div className="flex items-center justify-between p-6 border-b bg-gradient-to-r from-purple-50 to-blue-50">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-gray-800">Inference Analytics</h2>
          </div>
