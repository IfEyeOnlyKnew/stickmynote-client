...
export function PadChatPanel({
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch(`/api/inference-pads/${padId}/messages`, {
        method: "POST",
  ...
  const sendTypingStatus = useCallback(async (typing: boolean) => {
    try {
      await fetch(`/api/inference-pads/${padId}/chat-typing`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const fetchTypingUsers = useCallback(async () => {
    try {
      const response = await fetch(`/api/inference-pads/${padId}/chat-typing`)
      if (response.ok) {
        const data = await response.json()
  ...
  const handlePinMessage = async (messageId: string) => {
    try {
      const response = await fetch(`/api/inference-pads/${padId}/messages/${messageId}/pin`, {
        method: "POST",
      })
      if (response.ok) {
        setMessages((prev) =>
  ...
  const handleDeleteMessage = async (messageId: string) => {
    ...
    try {
      const response = await fetch(`/api/inference-pads/${padId}/messages/${messageId}`, {
        method: "DELETE",
      })
      if (response.ok) {
  ...
  const handleToggleReaction = async (messageId: string, emoji: ReactionEmoji) => {
    try {
      await fetch(`/api/inference-pads/${padId}/messages/${messageId}/reactions`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
