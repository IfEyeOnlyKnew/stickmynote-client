...
export function PadQADialog({ padId, padName, trigger }: PadQADialogProps) {
  ...
  const fetchHistory = async () => {
    ...
    try {
      const response = await fetch(`/api/social-pads/${padId}/qa-history?limit=20`)
      if (response.ok) {
  ...
  const handleAsk = async () => {
    ...
    try {
      const response = await fetch(`/api/social-pads/${padId}/ask`, {
        method: "POST",
  ...
  const handleFeedback = async (wasHelpful: boolean) => {
    ...
    try {
      await fetch(`/api/social-pads/qa/${currentQAId}/feedback`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
