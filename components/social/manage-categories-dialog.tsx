...
export function ManageCategoriesDialog({ open, onOpenChange, onUpdate }: ManageCategoriesDialogProps) {
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch("/api/social-pad-categories")
      if (response.ok) {
  ...
  const handleUpdate = async (categoryId: string) => {
    ...
    try {
      setLoading(true)
      const response = await fetch(`/api/social-pad-categories/${categoryId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: editName,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Organize your Social Pads into categories. Categories help group related pads together in Public and Private
            views.
          </DialogDescription>
        </DialogHeader>
