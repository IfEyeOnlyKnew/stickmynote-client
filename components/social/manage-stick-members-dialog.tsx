...
export function ManageStickMembersDialog({
  ...
  const fetchMembers = async () => {
    try {
      const response = await fetch(`/api/social-sticks/${stickId}/members`)
      if (response.ok) {
  ...
  const handleRemoveMember = async (memberId: string) => {
    try {
      const response = await fetch(`/api/social-sticks/${stickId}/members/${memberId}`, {
        method: "DELETE",
      })
  ...
  const handleCsvUpload = async (emails: Array<{ email: string; name?: string }>) => {
    try {
      ...
      const response = await fetch(`/api/social-sticks/${stickId}/members/bulk`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          emails: emailList,
