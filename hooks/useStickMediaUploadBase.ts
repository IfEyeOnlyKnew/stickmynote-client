...
interface UseStickMediaUploadBaseProps {
  stickId: string
...
interface UseStickMediaUploadBaseReturn<T extends MediaItem> {
  // Common state
  url: string
...
export function useStickMediaUploadBase<T extends MediaItem>({
  stickId,
  config,
  onTabsUpdate,
  onTabChange,
  tabType,
  tabName,
}: UseStickMediaUploadBaseProps): UseStickMediaUploadBaseReturn<T> {
  ...
  const handleAdd = async (existingItems: T[], newItem: T): Promise<void> => {
    ...
    setIsLoading(true)
    try {
      console.log("[v0] Adding stick media item:", { stickId, tabType, newItem })
      ...
      console.log("[v0] Saving updated stick items:", updatedItems.length)

      await config.saveStickTab(stickId, tabType, { [tabType]: updatedItems })

      await new Promise((resolve) => setTimeout(resolve, 500))

      try {
        const updatedTabs = await config.getStickTabs(stickId)
        console.log("[v0] Refreshed stick tabs:", updatedTabs.length)
        onTabsUpdate(updatedTabs)
      } catch (refreshError) {
        console.warn("[v0] Failed to refresh stick tabs, but save was successful:", refreshError)
      }
      ...
      showSuccess(`${tabName.slice(0, -1)} added`, `${tabName.slice(0, -1)} saved to this stick.`)
    } catch (error) {
      console.error("[v0] Error adding stick media item:", error)
  ...
  const handleDelete = async (itemId: string): Promise<void> => {
    ...
    try {
      ...
      await config.deleteStickTabItem(stickId, tabType, itemId)

      await new Promise((resolve) => setTimeout(resolve, 500))

      console.log("[v0] Refreshing stick tabs after delete...")
      const updatedTabs = await config.getStickTabs(stickId)
      console.log("[v0] Updated stick tabs received:", updatedTabs.length)
      onTabsUpdate(updatedTabs)

      onTabChange?.(tabName)
      showSuccess(`${tabName.slice(0, -1)} deleted`, "Item successfully removed.")
    } catch (error) {
      console.error("[v0] Error deleting stick media item:", error)
      const errorMessage = error instanceof Error ? error.message : "Unknown error occurred"
