// v2 Organization Pad Calsticks API: production-quality, list calendar stick events for a pad within an organization
import { NextRequest } from 'next/server'
import { requireADSession } from '@/lib/auth/ad-session'
import { query } from '@/lib/database/pg-helpers'
import { handleApiError } from '@/lib/api/handle-api-error'

// GET /api/v2/organizations/[orgId]/pads/[padId]/calsticks - List calendar stick events for a pad within an organization
export async function GET(request: NextRequest, { params }: { params: Promise<{ orgId: string, padId: string }> }) {
  try {
    ...
    const events = await query(
      `SELECT * FROM calsticks WHERE pad_id = $1 AND org_id = $2 ORDER BY start_time DESC`,
      [padId, orgId]
    )
