// v2 Organization Multipaks API: production-quality, list multipaks for an organization
import { NextRequest } from 'next/server'
import { requireADSession } from '@/lib/auth/ad-session'
import { query } from '@/lib/database/pg-helpers'
import { handleApiError } from '@/lib/api/handle-api-error'

// GET /api/v2/organizations/[orgId]/multipaks - List multipaks for an organization
export async function GET(request: NextRequest, { params }: { params: { orgId: string } }) {
  try {
    ...
    const multipaks = await query(
      `SELECT * FROM multipaks WHERE org_id = $1 ORDER BY created_at DESC`,
      [orgId]
    )
