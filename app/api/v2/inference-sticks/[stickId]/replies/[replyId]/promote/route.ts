...
// POST /api/v2/inference-sticks/[stickId]/replies/[replyId]/promote - Promote reply to CalStick
export async function POST(
  ...
  try {
    ...
    const inferenceStick = stickResult.rows[0]

    // Get the social pad name to use for the paks_pad
    const inferencePadResult = await db.query(
      `SELECT id, name FROM social_pads WHERE id = $1`,
      [inferenceStick.social_pad_id]
    )

    if (inferencePadResult.rows.length === 0) {
      return new Response(JSON.stringify({ error: 'Social pad not found' }), { status: 404 })
    }

    const inferencePad = inferencePadResult.rows[0]
