...
export async function POST(request: NextRequest, { params }: { params: Promise<{ stickId: string }> }) {
  try {
    ...
    if (existingTabs) {
      ...
      if (error) {
        console.error("[POST /api/inference-sticks/tabs] Update Error:", error)
        return NextResponse.json({ error: error.message }, { status: 500 })
      }
      ...
      if (error) {
        console.error("[POST /api/inference-sticks/tabs] Insert Error:", error)
        return NextResponse.json({ error: error.message }, { status: 500 })
      }
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ stickId: string }> }) {
  try {
    ...
    if (tabId) {
      ...
      if (selectError) {
        console.error("[DELETE /api/inference-sticks/tabs] Select Error:", selectError)
