...
export async function POST(request: Request, { params }: { params: Promise<{ stickId: string }> }) {
  try {
    ...
    const inferencePads = { owner_id: pad.owner_id, name: pad.name }
    const hasPermission = await checkPermissions(db, { social_pad_id: stick.social_pad_id, social_pads: inferencePads }, user.id)
    if (!hasPermission) {
