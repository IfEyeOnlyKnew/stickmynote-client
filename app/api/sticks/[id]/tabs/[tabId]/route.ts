...
async function checkStickPermissions(supabase: any, stickId: string, userId: string) {
  ...
  const { data: pad } = await supabase.from("paks_pads").select("owner_id").eq("id", stick.pad_id).maybeSingle()
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string; tabId: string }> }) {
  try {
    ...
    // Check permissions
    const hasPermission = await checkStickPermissions(supabase, stickId, user.id)
    if (!hasPermission) {
      return NextResponse.json({ error: "Permission denied" }, { status: 403 })
    }

    const { error } = await supabase.from("paks_pad_stick_tabs").delete().eq("id", tabId).eq("stick_id", stickId)
