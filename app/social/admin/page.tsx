...
export default function SocialHubAdminPage() {
  ...
  const fetchAdminData = async () => {
    try {
      ...
      // Fetch all pads
      const padsRes = await fetch("/api/social-pads?admin=true")
      const padsData = await padsRes.json()
      setPads(padsData.pads || [])

      // Fetch all sticks
      const sticksRes = await fetch("/api/social-sticks?admin=true")
  ...
  const togglePadVisibility = async (padId: string, isPublic: boolean) => {
    try {
      await fetch(`/api/social-pads/${padId}`, {
        method: "PATCH",
  ...
  const deletePad = async (padId: string) => {
    ...
    try {
      await fetch(`/api/social-pads/${padId}`, { method: "DELETE" })
      fetchAdminData()
    } catch (error) {
  ...
  const deleteStick = async (stickId: string) => {
    ...
    try {
      await fetch(`/api/social-sticks/${stickId}`, { method: "DELETE" })
      fetchAdminData()
    } catch (error) {
    ...
    <div className="container mx-auto py-8 px-4">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-4xl font-bold">Social Hub Administration</h1>
        </div>
        <p className="text-muted-foreground">Manage Social Hub settings, content, and administrators</p>
        {currentUserRole && (
      ...
      <Card className="mb-6 border-primary/20 bg-primary/5">
        <CardContent className="py-4">
          <div className="flex items-center justify-between">
            ...
            <Button asChild>
              <Link href="/social/admin/cleanup-policies">
                Open Dashboard
      ...
      <Tabs defaultValue="pads" className="space-y-6">
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="pads">
            ...
            Social Pads ({filteredPads.length})
          </TabsTrigger>
          <TabsTrigger value="sticks">
            <FileText className="h-4 w-4 mr-2" />
            Social Sticks ({filteredSticks.length})
          </TabsTrigger>
