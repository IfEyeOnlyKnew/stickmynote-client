...
export default function SocialHubsPage() {
  ...
  const renderHubCards = (hubs: SocialPad[], iconColor: string) => (
        ...
        <Card key={pad.id} className="hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            ...
            <div className="flex gap-2">
              <Button onClick={() => router.push(`/social/pads/${pad.id}`)} className="flex-1" variant="outline">
                View Pads
                <ArrowRight className="h-4 w-4 ml-2" />
              </Button>
              {(pad.user_role === "owner" || pad.user_role === "admin") && (
                <Button onClick={() => router.push(`/social/pads/${pad.id}/edit`)} variant="ghost" size="icon">
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        <BreadcrumbNav
          items={[
            ...
            { label: "Social Hubs", current: true },
          ]}
        />

        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Social Hubs</h1>
          <p className="text-gray-600 text-lg">Manage your Individual and Organization Social Hubs</p>
        </div>
