...
// --- Small presentational helpers. Extracted so DeployGuidePage stays under
//     SonarCloud's S3776 cognitive-complexity limit (was 25, target ≤15).

function StatusIndicator({ ok }: Readonly<{ ok?: boolean }>) {
  if (ok) {
    return (
...
function StatusCard({
  ...
  return (
    <div className="border rounded p-3">
      ...
      <ul className="mt-2 text-sm text-gray-600 space-y-1">{children}</ul>
      {footer && <div className="mt-2">{footer}</div>}
    </div>
  )
}

function PostgresCard({ env }: Readonly<{ env: EnvStatus | null }>) {
  const present = env?.hasPostgresHost && env?.hasPostgresDatabase
  return (
    <StatusCard
      title="PostgreSQL env"
      badge={{ label: present ? "Present" : "Missing", variant: present ? "default" : "secondary" }}
    >
      <li>Host: <StatusIndicator ok={env?.hasPostgresHost} /></li>
      <li>Database: <StatusIndicator ok={env?.hasPostgresDatabase} /></li>
      <li>User: <StatusIndicator ok={env?.hasPostgresUser} /></li>
    </StatusCard>
  )
}

function RateLimiterCard({ rate }: Readonly<{ rate: RateHealth | null }>) {
  const healthy = rate?.redis || rate?.fallback
  return (
    <StatusCard
      title="Rate limiter"
      badge={{
        label: (rate?.provider ?? "unknown").toString(),
    ...
    >
      <li>Redis connected: <StatusIndicator ok={rate?.redis} /></li>
      <li>Fallback available: <StatusIndicator ok={rate?.fallback} /></li>
      {rate?.warning ? <li className="text-yellow-700">Note: {rate.warning}</li> : null}
    </StatusCard>
  )
}

function SeoCard({ seo }: Readonly<{ seo: { robots: boolean; sitemap: boolean } | null }>) {
  const healthy = seo?.robots && seo?.sitemap
  return (
    <StatusCard
      title="SEO endpoints"
      badge={{ label: healthy ? "OK" : "Check", variant: healthy ? "default" : "secondary" }}
      footer={
    ...
    >
      <li>robots.txt: <StatusIndicator ok={seo?.robots} /></li>
      <li>sitemap.xml: <StatusIndicator ok={seo?.sitemap} /></li>
    </StatusCard>
  )
}

function DatabaseCard({ db }: Readonly<{ db: { ok?: boolean; message?: string } | null }>) {
  return (
    <StatusCard
      title="Database health"
      badge={{ label: db?.ok ? "OK" : "Check", variant: db?.ok ? "default" : "secondary" }}
      footer={
        <Link
...
function QuickStatusSection({
  ...
  loading: boolean
  env: EnvStatus | null
  rate: RateHealth | null
  seo: { robots: boolean; sitemap: boolean } | null
  db: { ok?: boolean; message?: string } | null
}>) {
  if (loading) return <p className="text-sm text-gray-500">Running checks...</p>
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <PostgresCard env={env} />
        <RateLimiterCard rate={rate} />
        <SeoCard seo={seo} />
        <DatabaseCard db={db} />
      </div>
...
// Hook: runs the five health checks once on mount and exposes the results
// plus a loading flag. Extracted so the page component doesn't carry its
// own useEffect + state plumbing.
function useDeployHealthChecks() {
  ...
  return { env, rate, seo, db, loading }
}

export default function DeployGuidePage() {
  const { env, rate, seo, db, loading } = useDeployHealthChecks()
  ...
  if (!isAccessible) return <NotAccessibleView />
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        <Card className="mb-6">
          ...
          <CardContent className="space-y-4">
            <QuickStatusSection loading={loading} env={env} rate={rate} seo={seo} db={db} />
          </CardContent>
        </Card>
