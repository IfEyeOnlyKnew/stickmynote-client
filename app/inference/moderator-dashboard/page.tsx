...
export default function ModeratorDashboardPage() {
  ...
  const fetchPads = async () => {
    try {
      const response = await fetch("/api/inference-pads/my-moderated-pads")
      if (response.ok) {
  ...
  useEffect(() => {
    if (!loading && !user) {
      router.push("/auth/login?redirectTo=/inference/moderator-dashboard")
      return
    }
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-4 ml-4">
                      ...
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation()
                          router.push(`/inference/pads/${pad.id}`)
                        }}
                      >
