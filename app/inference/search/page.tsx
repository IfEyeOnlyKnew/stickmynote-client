...
export default function InferenceSearchPage() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const { user, loading: userLoading } = useUser()
  const [query, setQuery] = useState(searchParams.get("q") || "")
  const [stickResults, setStickResults] = useState<InferenceStick[]>([])
  ...
  const performSearch = async (searchQuery = query) => {
    ...
    try {
      ...
      const response = await fetch(`/api/inference-search?${params}`)
      if (response.ok) {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault()
    if (query.trim()) {
      router.push(`/inference/search?q=${encodeURIComponent(query)}`)
    }
    performSearch()
  }
    ...
    if (replyResults.length > 0) {
      return replyResults.map((reply) => (
        <Card
          key={reply.id}
          className="cursor-pointer hover:shadow-lg transition-shadow"
          onClick={() => router.push(`/inference/sticks/${reply.social_stick_id}`)}
        >
          <CardHeader>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Enhanced Inference Search</h1>
