...
import { CleanupPolicySettings } from "@/components/inference/cleanup-policy-settings"
...
export default function CleanupPoliciesDashboard() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const padsRes = await fetch("/api/inference-pads?admin=true&include_policies=true")
      ...
      for (const pad of padsData.pads || []) {
        ...
        const policyRes = await fetch(`/api/inference-pads/${pad.id}/cleanup-policy`)
        const policyData = await policyRes.json()

        // Get stick counts
        const sticksRes = await fetch(`/api/inference-sticks?pad_id=${pad.id}&include_archived=true`)
  ...
  const runManualCleanup = async () => {
    ...
    try {
      const res = await fetch("/api/cron/cleanup-inference-sticks", {
        method: "POST",
  ...
  const toggleQuickPolicy = async (padId: string, field: string, value: boolean) => {
    try {
      ...
      const res = await fetch(`/api/inference-pads/${padId}/cleanup-policy`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <div className="flex items-center justify-between mb-6">
        <BreadcrumbNav
          items={[
            ...
            { label: "Administration", href: "/inference/admin" },
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Manage automatic content cleanup policies across all Inference Hub pads</p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure cleanup rules for each Inference Hub pad. Toggle quick settings or open full configuration.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
