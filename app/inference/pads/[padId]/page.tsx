...
import { PadSettingsDialog } from "@/components/inference/pad-settings-dialog"
import { CreateStickModal } from "@/components/create-stick-modal"
import { ManageStickMembersDialog } from "@/components/inference/manage-stick-members-dialog"
...
export default function InferencePadPage({ params }: Readonly<{ params: { padId: string } }>) {
  ...
  useEffect(() => {
    const processPendingInvites = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/inference-pads/${padId}/process-pending-invites`, {
          method: "POST",
        })
  ...
  const fetchPadData = async () => {
    try {
      ...
      if (response.status === 401) {
        router.push(`/auth/login?redirectTo=/inference/pads/${padId}`)
        return
      }
      ...
      if (response.ok) {
        ...
        const sticksWithReactions = await Promise.all(
          (data.sticks || []).map(async (stick: InferenceStick) => {
            try {
              const reactionsRes = await fetch(`/api/inference-sticks/${stick.id}/reactions`)
              if (reactionsRes.ok) {
  ...
  const handlePinToggle = async (e: React.MouseEvent, stickId: string) => {
    ...
    try {
      const response = await fetch(`/api/inference-sticks/${stickId}/pin`, {
        method: "POST",
      })
  ...
  const handleManageStickMembers = (e: React.MouseEvent, stick: InferenceStick) => {
    e.stopPropagation()
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-4 py-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mt-4 gap-3">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl inference-gradient flex items-center justify-center shadow-lg flex-shrink-0">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {isOwner && (
                <>
                  <Button variant="outline" size="sm" onClick={() => router.push(`/inference/pads/${padId}/analytics`)} className="hidden md:flex">
                  ...
                  <Button variant="outline" size="sm" onClick={() => router.push(`/inference/pads/${padId}/analytics`)} className="md:hidden">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          {user && canAddSticks && (
            <Button onClick={() => setCreateStickOpen(true)} className="inference-gradient text-white w-full sm:w-auto">
        ...
        {sticks.length === 0 ? (
          <Card className="border-2 border-dashed border-purple-200 bg-white/50 backdrop-blur-sm">
            <CardContent className="py-16 text-center">
              <div className="w-20 h-20 rounded-full inference-gradient mx-auto mb-6 flex items-center justify-center">
              ...
              {user && canAddSticks && (
                <Button
                  ...
                  size="lg"
                  className="inference-gradient text-white shadow-lg"
                >
      ...
      <CreateStickModal isOpen={createStickOpen} onClose={handleStickCreated} padId={padId} context="inference" />
